...
class LeftSideBar extends Component {
    ...
    render(){
        const {symbol, item, favImage, storeName, addToCart, addToCartUnsafe, addToWishlist} = this.props
        var products = {
            slidesToShow: 1,
            slidesToScroll: 1,
            dots: false,
        ...
        return (
            <div>
                {/*SEO Support*/}
                <Helmet>
                    <title>{storeName} | {item.name}</title>
                                        ...
                                        <div className="row">
                                            <div className="col-lg-6 product-thumbnail">
                                                <Slider {...products} asNavFor={this.state.nav2} ref={slider => (this.slider1 = slider)} className="product-slick">
                                                    ...
                                                    (item.secondaryImageModel).length?
                                                    item.secondaryImageModel.map((vari, index) =>
                                                        <div key={index}>
...
const mapStateToProps = (state, ownProps) => {
    let productId = ownProps.match.params._id;
    return {
        item: state.data.products.find(el => el._id == productId),
